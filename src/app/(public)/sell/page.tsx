...
import { Info, Star, TrendingUp, Users, Zap } from "lucide-react";
...
import { marketReferenceRate, marketRateUpdatedAt, sellPageHighlights } from "@/lib/mock/offers";
import { formatTime } from "@/lib/date";

export const metadata: Metadata = {
  title: "Sell RMB",
  description:
    "Choose from verified buyers and sell RMB at the best rates on ScarExchange.",
};

export default async function SellRmbPage() {
  const offers = await getOffers({ side: "buy", sort: "best_rate" });
    ...
    <div className="mx-auto max-w-[1400px] px-4 py-8 sm:px-6">
      <header className="flex flex-wrap items-start justify-between gap-6">
        <div>
          ...
          <p className="mt-1 text-[13.5px] text-neutral-500 dark:text-neutral-400">
            Choose from verified buyers and sell RMB at the best rates.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Highlight
            ...
            value={formatDuration(sellPageHighlights.fastestReleaseSeconds)}
            tone="brand"
          />
          <Highlight
            icon={Star}
            label="Top Rated"
            value={sellPageHighlights.topRating.toFixed(2)}
            tone="warning"
          />
          <Highlight
            icon={Users}
            label="Active Buyers"
            value={String(sellPageHighlights.activeBuyers)}
            tone="brand"
          />
      ...
      <div className="mt-7">
        <OfferBrowser
          offers={offers}
          side="buy"
          totalCount={sellPageHighlights.activeBuyers}
        />
      </div>
